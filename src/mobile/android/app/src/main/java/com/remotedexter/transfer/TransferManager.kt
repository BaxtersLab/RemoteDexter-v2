...
class TransferManager(
    ...
    fun handleCommand(command: String, data: ByteArray) {
        try {
            when (command) {
                "file_offer" -> {
                    val offer = com.remotedexter.protocol.deserialize<FileOffer>(data)
                    fileReceiver.handleFileOffer(offer)
                }
                "file_accept" -> {
                    val accept = com.remotedexter.protocol.deserialize<FileAccept>(data)
                    fileSender.handleFileAccept(accept)
                }
                "file_reject" -> {
                    val reject = com.remotedexter.protocol.deserialize<FileReject>(data)
                    fileSender.handleFileReject(reject)
                }
                "file_chunk" -> {
                    val chunk = com.remotedexter.protocol.deserialize<FileChunk>(data)
